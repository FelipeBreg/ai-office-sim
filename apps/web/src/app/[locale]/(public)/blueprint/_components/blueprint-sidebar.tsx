...
import type { BlueprintTopic } from '../_lib/blueprint-topics';

function TopicItem({ topic, active }: { topic: BlueprintTopic; active: boolean }) {
      ...
      {hasSubtopics && isExpanded && (
        <div className="ml-5 border-l border-border-default">
          {topic.subtopics!.map((sub) => (
            <Link
              key={sub.id}
              href={`/blueprint/${topic.slug}#${sub.id}`}
...
export default function BlueprintSidebar() {
  ...
  const currentSlug = pathname.replace(/^\/blueprint\/?/, '').split('#')[0] || 'visao-geral';
